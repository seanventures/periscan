...
for (const requirement of requirements) {
  ...
  if (
    ...
    for (const reference of requirement.evidenceRefs) {
      const evidencePath = resolve(root, "docs/qa", reference);
      if (!existsSync(evidencePath)) {
        // lab-runs/ is gitignored and PUBLIC_TREE-excluded; skip when absent
        // so stranger clones can still pass pnpm verify / analyst:score:check.
        if (String(reference).startsWith("lab-runs/")) {
          continue;
        }
...
// Totals: 1,490/1,880 (79.3) after 2026-08-03 hybrid-plant ops lift on row #30
// (Partial 3.75, ops=4, not Strong). Slice F catch-up: 1,489/1,880 (79.2).
// Prior Slice E: 1,484/1,880 (78.9). Slice D: 1,471/1,880 (78.2). Slice A: 1,383/1,880 (73.6).
//
...
if (currentPoints !== 1490) {
  fail(
    `current score must reconcile to 1490/1880; received ${currentPoints}/1880`
  );
}
