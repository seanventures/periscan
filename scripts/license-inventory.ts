...
function evaluateSimpleLicensePolicy(
  ...
  // REVIEW_LICENSE_PATTERNS exclude AGPL (`[^a]gpl`). Skipping Blocked without
  // returning here would fall through to Allowed — the certify mismatch.
  if (isNamedAgplReviewEntry(input, license)) {
    return {
      disposition: "RequiresLegalReview",
      reason:
        "Named AGPL exception: TruffleHog is quarantined behind legal review (Engine Lab accept→install). Not Allowed. Other AGPL stays Blocked."
    };
  }
