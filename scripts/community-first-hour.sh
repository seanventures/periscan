...
# Host Redis/MinIO listeners (SSH tunnels or an older compose project) collide
# with published ports. Prefer reuse; otherwise remap.
if docker ps --format '{{.Names}} {{.Ports}}' | grep -q '5434->5432'; then
  echo "==> postgres already published on :5434 — skipping compose create"
else
  if lsof -nP -iTCP:6379 -sTCP:LISTEN >/dev/null 2>&1; then
    export PERISCAN_REDIS_PUBLISHED_PORT="${PERISCAN_REDIS_PUBLISHED_PORT:-6384}"
    export REDIS_URL="redis://127.0.0.1:${PERISCAN_REDIS_PUBLISHED_PORT}"
    echo "==> :6379 busy — Redis publish ${PERISCAN_REDIS_PUBLISHED_PORT}"
  fi
  if lsof -nP -iTCP:9000 -sTCP:LISTEN >/dev/null 2>&1; then
    export PERISCAN_MINIO_PUBLISHED_PORT="${PERISCAN_MINIO_PUBLISHED_PORT:-9010}"
    echo "==> :9000 busy — MinIO publish ${PERISCAN_MINIO_PUBLISHED_PORT}"
  fi
...
Then in another terminal, operator TUI against PERISCAN_API_URL:
