...
echo "==> e2e"
if pgrep -fl 'next/dist/bin/next' | grep -q "$ROOT_DIR/apps/web"; then
  echo "error: next dev already running in this checkout ($ROOT_DIR/apps/web)." >&2
  echo "Stop it (or use another worktree) so Playwright can start CSRF-off e2e servers." >&2
  exit 1
fi
CI=1 pnpm test:e2e
