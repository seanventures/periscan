...
One paste (installs toolchain, clones if needed, starts, health-checks):
...
a hosted `github.com/org/repo` URL). Run Community validation. Remediations stay
Open until a retest verifies.

Requires **Node 24** (`.nvmrc`), **pnpm 9.15.0** (Corepack), and **Docker**.
