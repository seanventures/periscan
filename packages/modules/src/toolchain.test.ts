...
describe("open source toolchain registry", () => {
  it("defaults listings to current tools, excluding deferred and legal-review-only items", () => {
    ...
    expect(defaultTools.some((tool) => tool.toolId === "atomic-red-team")).toBe(
      false
    );
    expect(
      defaultTools.some((tool) => tool.toolId === "invoke-atomicredteam")
    ).toBe(false);
  ...
  it("can include deferred and legal-review-only items when explicitly requested", () => {
    ...
    expect(
      allTools.some((tool) => tool.toolId === "invoke-atomicredteam")
    ).toBe(true);
