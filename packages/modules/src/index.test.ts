...
describe("module registry", () => {
  ...
  it("derives licenseRisk from tool disposition, not only module.license (P15-4)", () => {
    ...
    // Named TruffleHog exception: AGPL Engine Lab path, never Allowed.
    expect(
      deriveLicenseRisk({
        license: "AGPL-3.0",
        toolIds: ["trufflehog"]
    ...
    const trufflehog = getModuleById("trufflehog.repo_secrets");
  ...
  it("runs the gitleaks wrapper against the fixture repository without leaking raw secrets", async () => {
    ...
    ).toEqual(["Exposure:SecretExposure"]);
    expect(output.signals[0]?.rawPayloadPointer).toMatch(/file=/);
    expect(output.signals[0]?.rawPayloadPointer).toMatch(/rule=/);
    expect(output.signals[0]?.rawPayloadPointer).not.toMatch(/Secret=/);
    expect(JSON.stringify(output.signals)).not.toContain(
