...
function isNamedAgplReviewTool(tool: OpenSourceToolDefinition): boolean {
  return (
    /agpl/iu.test(tool.license) &&
    tool.policyStatus === "RequiresLegalReview" &&
    (tool.toolId === "trufflehog" || tool.toolId.startsWith("trufflehog."))
  );
}

function licenseStringToRisk(
  license: string,
  namedAgplReview = false
): z.infer<typeof ModuleLicenseRiskSchema> {
  if (namedAgplReview && /agpl/iu.test(license)) {
...
export function deriveLicenseRisk(
  ...
  const namedAgplReview = tools.some(isNamedAgplReviewTool);
  let risk = licenseStringToRisk(manifest.license, namedAgplReview);
  for (const tool of tools) {
    risk = maxLicenseRisk(
      risk,
      licenseStringToRisk(tool.license, isNamedAgplReviewTool(tool))
    );
...
function createGitleaksSignals(
  ...
  return findings.map((finding) => {
    ...
    params.set("file", finding.File);
    if (finding.RuleID) {
      params.set("rule", finding.RuleID);
    }
    if (finding.StartLine) {
      params.set("line", String(finding.StartLine));
    }
    const rawPayloadPointer = `gitleaks://${repositoryLabel}?${params.toString()}`;
  ...
  createModule(
    ...
    async (context) => {
      ...
      return {
        ...
        // SETTLED: first clean observation is Validated, never Fixed.
        // RemediationTask Fixed remains verify-only (PERISCAN-545).
        validationState: "Validated",
        signals: createGitleaksSignals(context, findings, repositoryLabel),
        evidence,
        errors: []
      };
    }
