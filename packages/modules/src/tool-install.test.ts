...
describe("third-party tool install planning", () => {
  ...
  it("selects npx as an installable runtime for promptfoo", () => {
    const promptfoo = getOpenSourceToolDefinition("promptfoo");

    expect(promptfoo).not.toBeNull();
    expect(selectOpenSourceToolInstallRuntime(promptfoo!, "npx")).toBe("npx");
  });
  ...
  it("rejects check-only runtime install requests", () => {
    ...
    expect(() =>
      selectOpenSourceToolInstallRuntime(gitleaks!, "binary")
    ).toThrow(/check-only/);
  });
