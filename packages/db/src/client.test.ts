...
import { createPrismaClient, TEST_PRISMA_CONNECTION_LIMIT } from "./client.js";
...
describe("createPrismaClient", () => {
  ...
  it("caps connection_limit under Vitest so parallel acc files fit postgres max_connections", () => {
    process.env.DATABASE_URL =
      "postgresql://user:pass@127.0.0.1:5432/periscan-db";
    ...
    expect(prismaClientMock).toHaveBeenCalledWith({
      datasourceUrl: `postgresql://user:pass@127.0.0.1:5432/periscan-db?connection_limit=${TEST_PRISMA_CONNECTION_LIMIT}`
    });
  });

  it("does not rewrite a non-test DATABASE_URL with a test connection_limit", () => {
