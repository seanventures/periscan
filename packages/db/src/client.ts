...
/** Vitest/acc workers share one postgres (`max_connections` often 100). */
export const TEST_PRISMA_CONNECTION_LIMIT = 3;
...
export function createPrismaClient() {
  ...
  if (process.env.VITEST || process.env.NODE_ENV === "test") {
    return createRlsExtendedClient(
      new PrismaClient({
        datasourceUrl: appendQueryParam(
          process.env.DATABASE_URL,
          "connection_limit",
          String(TEST_PRISMA_CONNECTION_LIMIT)
        )
      })
    );
  }
