...
describe("Community edition validation suite", () => {
  ...
  it("infers hosted git org/repo pastes as Repository, not Domain", () => {
    ...
    expect(
      inferCommunityScopeType(
        "https://git-codecommit.us-east-1.amazonaws.com/v1/repos/payments-api"
      )
    ).toBe("Repository");
    expect(
      inferCommunityScopeType("https://gist.github.com/acme/payments-api")
    ).toBe("Repository");
    expect(inferCommunityScopeType("https://app.example.com/login")).toBe(
      "Domain"
    );
    expect(inferCommunityScopeType("/opt/repo")).toBe("Repository");
  });
...
describe("Community first-hour pack (PERISCAN-567)", () => {
  const startableRepo = [
    "gitleaks.repo_secrets",
    "trivy.repo_dependency_scan",
  ...
  it("is Gitleaks-class secrets only, not the 38-engine pack", () => {
    expect(COMMUNITY_GITLEAKS_REPO_SECRETS_MODULE_ID).toBe(
      "gitleaks.repo_secrets"
    );
    expect([...COMMUNITY_FIRST_HOUR_MODULE_IDS]).toEqual([
      "gitleaks.repo_secrets"
    ]);
    expect(communityFirstHourStartModuleIds(startableRepo)).toEqual([
      "gitleaks.repo_secrets"
    ]);
...
describe("Community edition valueLine (PERISCAN-570)", () => {
  it("does not name Nuclei, Prowler, kube CIS, or recon as what a repo Run does", () => {
    ...
    expect(COMMUNITY_EDITION_VALUE_LINE).not.toMatch(/kube-bench/i);
    expect(COMMUNITY_EDITION_VALUE_LINE).not.toMatch(/\brecon\b/i);
    expect(COMMUNITY_EDITION_VALUE_LINE).not.toMatch(/\bnmap\b/i);
    expect(COMMUNITY_EDITION_VALUE_LINE).not.toMatch(/naabu/i);
    expect(COMMUNITY_EDITION_VALUE_LINE).not.toMatch(/amass/i);
  });
});
