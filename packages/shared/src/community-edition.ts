...
/**
 * Start set for unpinned first-hour. Empty when Gitleaks is not startable
 * (Domain / CloudAccount), so callers fall back to the scope's startable set.
 */
export function communityFirstHourStartModuleIds(
  startableModuleIds: readonly string[]
): string[] {
  const allowed = new Set(startableModuleIds);
  return COMMUNITY_FIRST_HOUR_MODULE_IDS.filter((id) => allowed.has(id));
}
...
/** Optional extra signal when a verified CloudAccount has zero startable engines. */
...
const GIT_FORGE_HOSTS = new Set(["github.com", "gitlab.com", "bitbucket.org"]);

function normalizeGitForgeHost(host: string): string {
  return host.toLowerCase().replace(/^www\./u, "");
}

function isGitForgeHost(host: string): boolean {
  const normalized = normalizeGitForgeHost(host);
  return (
    GIT_FORGE_HOSTS.has(normalized) ||
    normalized.endsWith(".github.com") ||
    normalized.includes("codecommit")
  );
}
...
/** github.com / gitlab.com / bitbucket.org / *.github.com / codecommit with org/repo. */
function hostedGitOrgRepo(value: string): boolean {
  const parsed = parseHostedGitValue(value);
  if (!parsed || !isGitForgeHost(parsed.host)) {
    return false;
  }
  if (normalizeGitForgeHost(parsed.host).includes("codecommit")) {
...
/**
 * Hosted github.com / *.github.com org/repo URLs are not a local clone path.
 * Infer still maps them to Repository; they cannot be the verified identifier.
 */
export function looksLikeHostedGitHubUrl(value: string): boolean {
  ...
  const parsed = parseHostedGitValue(trimmed);
  if (!parsed) {
    return false;
  }
  const host = normalizeGitForgeHost(parsed.host);
...
export const HOSTED_GITHUB_REPOSITORY_REFUSAL =
  "Repository scope value must be a local clone path the control plane or a runner can read, not a hosted GitHub URL.";
 ...
 * value is ambiguous — the operator must pick a type. Hosted git org/repo
 * URLs (github.com, gitlab.com, bitbucket.org, *.github.com, codecommit)
 * classify as Repository so Validate paste does not keep Type Domain.
 */
