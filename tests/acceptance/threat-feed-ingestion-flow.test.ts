...
describe("Threat feed ingestion acceptance workflow", () => {
  it("ingests CISA KEV, persists deduped advisories with feed provenance, and skips on re-run", async () => {
    ...
    try {
      ...
      // First ingestion imports both feed entries. feedUrl must resolve to a
      // public address (SSRF DNS runs before the injected fetch); .test does not.
      const firstIngest = await app.inject({
        cookies: authCookies(cookie),
        method: "POST",
        payload: {
          feed: "cisa_kev",
          feedUrl: "https://example.com/feeds/known.json"
        },
