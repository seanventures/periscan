...
function resultAuditHash(payload: Record<string, unknown>): string {
...
function signedResult(
  signer: { sign: (localAuditSha256: string) => string },
  payload: Record<string, unknown>
) {
  const unsigned: Record<string, unknown> = { signals: [], ...payload };
  const localAuditSha256 = resultAuditHash(unsigned);
  return {
    ...unsigned,
    localAuditSha256,
...
describe("runner lease-complete E2E (register → heartbeat → poll lease → complete)", () => {
  ...
  it("registers, heartbeats, leases a queued task, and completes with a signed result", async () => {
    ...
    try {
      ...
      const completePayload = signedResult(signer, {
      ...
      expect(done.localAuditHash).toBe(completePayload.localAuditSha256);
    } finally {
      await app.close();
    }
  }, 45_000);
