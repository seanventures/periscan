...
describe("API edge regression acceptance", () => {
  ...
  it("preserves policy-binding error codes, audit trail, resolved targets, and bad-cookie behavior", async () => {
    ...
    try {
      ...
      // ExternalPoA (nuclei) post-resolve DNS denies NXDOMAIN hostnames, so the
      // verified scope/target must be a public name. example.com is IANA-reserved
      // and resolves only to public addresses; tenant isolation still unique.
      const scopeValue = "example.com";
      ...
      // Queued missions are not restartable (409). Bind the target mismatch on a
      // fresh Draft mission that still carries the same policy decision.
      const overrideMissionResponse = await app.inject({
        cookies: testHelpers.authCookies(cookie),
        method: "POST",
        payload: {
          missionType: "ExposureValidation",
      ...
      expect(overrideMissionResponse.statusCode).toBe(201);
      const overrideMissionId = overrideMissionResponse.json()
        .missionId as string;
      ...
      const targetOverrideResponse = await app.inject({
        ...
        url: `/api/v1/missions/${overrideMissionId}/start`
      });
