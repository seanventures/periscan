...
describe("Slice D safety scaffolds + AI ops floors", () => {
  ...
  it("AI ops floors 59/61/64 stay safe-canary only (control + prompt injection + model extraction)", async () => {
    ...
    try {
      ...
      expect(fixture.statusCode, fixture.body).toBe(200);
      ...
      expect(promptInjection.statusCode, promptInjection.body).toBe(200);
      ...
      expect(extraction.statusCode, extraction.body).toBe(200);
      ...
      await app.close();
      await testHelpers.closeHttpServer(endpoint);
    }
  });
});
