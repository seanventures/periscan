...
describe("Community validation start queues a real mission (PERISCAN-505)", () => {
  ...
  it("POST /community/validation-runs on a verified Domain creates a mission and queues worker-lane work", async () => {
    ...
    try {
      ...
      // Companion mission may exist even when External PoA is denied (ControlPlane
      // policy, kill switch, rate, or hostname guard). Skip reason is the honesty
      // signal; mission id is optional audit/companion linkage.
      if (body.nucleiSkipReason) {
        expect(body.nucleiSkipReason).toMatch(
          /Nuclei|PoA|denied|did not start/i
        );
      } else {
        expect(body.nucleiMissionId).toBeTruthy();
      }
  ...
  it("POST /community/validation-runs on a verified Repository selects first-hour Gitleaks", async () => {
    const queued: QueuedJob[] = [];
    const harness = await buildAcceptanceApp(queued);
    prisma = harness.prisma;
    const { app } = harness;
    ...
    try {
      ...
      expect(started.json().mission.missionId).toBeTruthy();
      expect(started.json().moduleIds).toEqual(["gitleaks.repo_secrets"]);
      expect(started.json().jobsQueued).toBe(1);
