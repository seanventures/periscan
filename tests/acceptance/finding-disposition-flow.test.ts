...
describe("finding disposition flow", () => {
  ...
  it("sets, persists, audits, and clears an analyst disposition", async () => {
    ...
    try {
      ...
      // Unknown finding id → 404. Payload must be schema-valid (FP/Suppressed
      // require a reason code or note) so this exercises lookup, not Zod 400.
      const missing = await app.inject({
        cookies: auth,
        method: "POST",
        payload: { disposition: "Suppressed", reasonCode: "OutOfScope" },
