...
type SecurityControlConnectorCase = {
  authType: string;
...
function valueForAuthField(connectorKey: string, field: AuthField): unknown {
  ...
  if (
    ...
    lowerKey === "timeamount" ||
    lowerKey === "statuspollms"
  ) {
    return 25;
  }

  if (ARRAY_FIELDS.has(key)) {
    if (connectorKey === "aws-waf" && key === "scopes") {
      return ["REGIONAL"];
    }

    return [`${connectorKey}-${key}-scope`];
  }
  ...
  if (
    ...
    if (connectorKey === "proofpoint" && key === "endpoint") {
      return "all";
    }

    return `https://${connectorKey}.security-control.example.test`;
  }
  ...
  if (lowerKey === "query" || lowerKey.includes("query")) {
    return "source = periscan";
  }

  if (lowerKey === "index") {
    return "periscan-security";
  }

  if (lowerKey === "email") {
    return `${connectorKey}-readonly@security-control.example.test`;
  }
...
async function buildSecurityControlConnectorCases(): Promise<
  SecurityControlConnectorCase[]
> {
  const manifests = getConnectorCatalog().filter(
    (manifest) =>
      manifest.category === "SecurityControl" && manifest.connectable === true
  );

  const cases: SecurityControlConnectorCase[] = [];
...
describe("SecurityControl connector acceptance", () => {
  it("creates, encrypts, syncs, audits, and tenant-isolates all connectable security-control connectors through public APIs", async () => {
    const connectorCases = await buildSecurityControlConnectorCases();
    const categoryCatalog = getConnectorCatalog().filter(
      (manifest) => manifest.category === "SecurityControl"
    );
    ...
    expect(plannedCatalog.map((manifest) => manifest.connectorKey)).toContain(
    ...
    try {
      const { cookie, response: signup } = await performSignup(
        app,
        "security-control-connectors",
        "Security Control Connector Tenant"
      );
      ...
      for (const connectorCase of connectorCases) {
        ...
        if (health.statusCode !== 200) {
          throw new Error(
            `Expected ${connectorCase.connectorKey} health to succeed, got ${health.statusCode}: ${health.body}`
          );
        }
        ...
        if (synced.statusCode !== 200) {
          throw new Error(
            `Expected ${connectorCase.connectorKey} sync to succeed, got ${synced.statusCode}: ${synced.body}`
          );
        }
      ...
      const connectedIntegrations = trustSafety.json()
        .connectedIntegrations as Array<{
        category: string;
        healthStatus: string;
      ...
      const { cookie: otherCookie } = await performSignup(
        app,
        "security-control-connectors-other",
        "Security Control Connector Other Tenant"
      );
