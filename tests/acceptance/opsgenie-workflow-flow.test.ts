...
describe("Opsgenie workflow destination acceptance", () => {
  it("creates, redacts, syncs, routes remediation alerts, and tenant-isolates Opsgenie through the public API", async () => {
    ...
    try {
      const { cookie, response: signup } = await performSignup(
        app,
        "opsgenie-acceptance",
        "Opsgenie Acceptance Tenant"
      );
      const tenantId = signup.json().tenant.tenantId as string;
      const plaintextApiKey = "opsgenie-acceptance-api-key";
      ...
      expect(storedConfig.priority).toBe("P1");
      ...
      expect(ticket.json().ticket.ticketId).toMatch(/^OG-/u);
      ...
      expect(storedRemediation.ticketId).toMatch(/^OG-/u);
      ...
      expect(connected.permissionsUsed).toEqual(
        expect.arrayContaining(["account:read", "alerts:create"])
      );
