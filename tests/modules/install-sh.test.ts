...
describe("install.sh (PERISCAN-576)", { timeout: 20_000 }, () => {
  ...
  it("install.sh and periscan.sh start print local-path first-hour next step", () => {
    ...
    const wrapper = runInstall(["--dry-run"], {}, "scripts/install.sh");
  ...
  it("README fold leads with git clone + inspect; curl|bash is secondary after Node 24 and Docker", () => {
    ...
    const clone = "git clone https://github.com/seanventures/periscan.git";
    expect(fold).toContain(clone);
    expect(fold).toContain("bash scripts/periscan.sh install");
    expect(fold).toContain("bash scripts/periscan.sh start");
    expect(fold).toMatch(/Node 24/);
    expect(fold).toMatch(/\bDocker\b/);
    expect(fold.indexOf(clone)).toBeGreaterThanOrEqual(0);
    expect(fold.indexOf(clone)).toBeLessThan(fold.indexOf("bash scripts/periscan.sh install"));
    expect(readme.indexOf(clone)).toBeLessThan(readme.indexOf(curl));
    expect(readme.slice(0, readme.indexOf(curl))).toMatch(/Node 24/);
    expect(readme.slice(0, readme.indexOf(curl))).toMatch(/\bDocker\b/);
    expect(readme).toMatch(/local clone path/i);
    expect(readme).toContain("Apache-2.0");
    expect(readme).toContain("github.com/org/repo");
    expect(readme).toContain("git clone <your-repo>");
    expect(readme).toMatch(/Gitleaks-class secrets/i);
    expect(readme).toContain("jobsQueued=1");
  });
