...
test.describe("web app shell navigation", () => {
  ...
  // PERISCAN-490 / AppSec+Jobs a11y: skip-link must stay off-canvas at rest and
  // must not cover the rail wordmark when revealed on keyboard focus.
  test("keeps skip-link hidden until focus and clear of the wordmark", async ({
    page
  }) => {
    ...
    const atRest = await skipLink.evaluate((element) => {
    ...
    // Clipped / off-canvas: no painted box in the viewport at rest.
    expect(
      atRest.bottom <= 0 ||
        atRest.top >= 1_000_000 ||
        (atRest.width <= 1 && atRest.height <= 1)
    ).toBe(true);
