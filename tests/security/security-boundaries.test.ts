...
describe("security boundary regression suite", () => {
  ...
  it("rejects runner task results with inline unverified evidence manifest items", async () => {
    ...
    try {
      ...
      // Fail-closed: an unbound/dummy audit hash is 403 and never reaches
      // evidence ingestion. Prefer 403 over 400 runner_evidence_not_uploaded —
      // unsigned or hash-mismatched results must not be accepted.
      const unboundHashResponse = await app.inject({
        headers: {
          authorization: `Bearer ${runnerAuthToken}`
        },
        method: "POST",
      ...
      expect(unboundHashResponse.statusCode).toBe(403);
      expect(unboundHashResponse.json().code).toBe(
        "runner_result_audit_hash_mismatch"
      );

      const localAuditSha256 = resultAuditHash(unsignedResult);
