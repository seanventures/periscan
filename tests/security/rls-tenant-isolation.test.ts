...
describe("RLS tenant-isolation backstop actively enforces", () => {
  ...
  it("runWithTenantRls permits a correctly-scoped write and isolates reads", async () => {
    ...
    resetTenantRlsContext();
    const seededA = await prisma.auditEvent.create({ data: makeAudit(tenantA) });
    const seededB = await prisma.auditEvent.create({ data: makeAudit(tenantB) });
    ...
    // Probe by primary key: a metadata JSON filter with no tenant predicate
    // seq-scans audit_events (RLS OR is not indexable) and hangs this suite
    // against a shared lab DB with millions of rows.
    const visible = await runWithTenantRls(prisma, tenantA, (tx) =>
      tx.auditEvent.findMany({
        where: {
          auditEventId: {
            in: [seededA.auditEventId, seededB.auditEventId, created.auditEventId]
          }
        }
      })
    );
    expect(visible.map((row) => row.auditEventId).sort()).toEqual(
      [seededA.auditEventId, created.auditEventId].sort()
    );
