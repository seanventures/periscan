...
function isEvidenceBackedGitleaksWithoutPathProof(
  finding: ValidatedFinding
): boolean {
  if (finding.pathProof != null || finding.sourceEntityType === "AttackPath") {
    return false;
  }
  if (finding.evidenceIds.length === 0) {
    return false;
  }
  return finding.source.includes("gitleaks.repo_secrets");
}

function isPathLinkedFinding(finding: ValidatedFinding): boolean {
  // Synthetic gitleaks relatedPathIds are not AttackPath rows. Remapping
  // Validated → Discovered here split Home from Findings (PERISCAN-568).
  if (isEvidenceBackedGitleaksWithoutPathProof(finding)) {
    return false;
  }
  return (
 ...
 * Evidence-backed Gitleaks without pathProof is not a path claim even if
 * relatedPathIds holds a synthetic UUID.
 */
