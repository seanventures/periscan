...
describe("projectFindingClaimDisplay", () => {
  ...
  it("keeps Validated for evidence-backed Gitleaks without hop receipts", () => {
    const finding = baseFinding({
      evidenceIds: ["55555555-5555-4555-8555-555555555555"],
      relatedPathIds: ["442092e0-e174-5d96-8a11-aaaaaaaaaaaa"],
    ...
    expect(display.displayValidationState).toBe("Validated");
    expect(display.remapped).toBe(false);
    expect(display.claimDisplayLabel).toBeNull();
    expect(display.ariaLabel).toBe("Validation state Validated");
  });

  it("still remaps Gitleaks when it is an AttackPath finding without hop receipts", () => {
    const finding = baseFinding({
      evidenceIds: ["55555555-5555-4555-8555-555555555555"],
      relatedPathIds: [pathId],
      source: "gitleaks.repo_secrets.scan",
