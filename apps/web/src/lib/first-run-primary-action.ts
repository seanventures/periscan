 ...
 * optional extra signal. After MeasuredResult the API nextAction continues
 * Act → Verify → Prove — never Integrations "Connect a source".
 */
...
export function resolveCommunityStartPrimaryAction(
  ...
  const nextIsConnectAws = next?.label === COMMUNITY_FIRST_RUN_CONNECT_AWS_LABEL;
