 ...
 * - null = unset (programStarted defaults ON on first Command Center visit)
 */
 ...
 * Default ON when Command Center is showing (programStarted) and pref is unset.
 * Explicit "0" stays off. Empty GetStarted does not inherit the default.
 */
...
export function resolveFirstHourHomeAction(
  activation: Parameters<typeof resolveFirstRunPrimaryAction>[0]
): { href: string; label: string } {
  const action = resolveFirstRunPrimaryAction(activation, {
    href: "/scopes",
    label: "Authorize scope"
  });
  if (
    action.href === "/integrations" ||
    /connect a source/i.test(action.label)
  ) {
    return { href: "/scopes", label: "Authorize scope" };
  }
...
export function DashboardCommandCenter() {
  ...
  // SSR-safe: start off; hydrate from storage / programStarted default in effects.
  ...
  useEffect(() => {
    ...
    // Unset: wait until lists settle so GetStarted never inherits Monday default.
    if (!programListsReady) {
      return;
    }
    const resolved = resolveMondayModeDefault(programStarted, null);
    if (resolved.shouldPersistDefault) {
      writeMondayModePref(true);
    }
    setMondayMode(resolved.on);
    mondayHydratedRef.current = true;
  }, [programListsReady, programStarted]);

  const toggleMondayMode = useCallback(() => {
    setMondayMode((prev) => {
      const next = !prev;
      // Explicit on/off — exit writes "0" so programStarted default does not re-arm.
  ...
  const ctemHonesty = useMemo(
    () =>
      resolveCtemBoardHonesty({
        ...
        verifyStatus: ctem.data?.stages.find((s) => s.stage === "Verify")
          ?.status,
  ...
  // /getting-started is a live alias of this surface (not a second board).
  // Do not mount GettingStartedGuide here.
          ...
          <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              ...
              <h2 className="mt-1 text-base font-semibold text-ink">
                {postFindingPrimary.label}
              </h2>
              {postFindingPrimary.reason ? (
            ...
            {paths.loading ? (
                  ...
                  <div className="flex flex-col gap-3 p-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <Link
                        ...
                        className={buttonClassName({
                          size: "sm",
                          variant: postFindingPrimary ? "secondary" : "primary"
                        })}
                      >
                        Inspect weakest hop
                      </Link>
              ...
              <div className="p-4" data-testid="dashboard-paths-empty">
                <NotConfigured
                  ...
                  message={EMPTY_PATHS_PROOF_MESSAGE}
                  action={emptyPathsHomeAction}
                />
              </div>
            )}
          </Panel>
        ...
        <Panel>
          ...
          {paths.loading ? (
            ...
            <div className="p-4" data-testid="dashboard-paths-empty">
              <NotConfigured
                title="No priority attack paths yet"
                message={`${EMPTY_PATHS_PROOF_MESSAGE} Multi-hop FullyMeasured requires hop receipts — empty is honest, not a fake Measured path.`}
                action={emptyPathsHomeAction}
              />
            </div>
          ) : (
            ...
            <div className="flex flex-col gap-3 p-4">
              <div className="flex items-center gap-4">
                {ctemHonesty.showPercent ? (
                  ...
                  <div
                    className="flex min-h-[92px] w-[9.5rem] shrink-0 flex-col justify-center rounded-control border border-line bg-bg px-2.5 py-2"
                    data-testid="dashboard-ctem-honesty"
                    role="status"
                  >
                      ...
                      <span
                        ...
                        style={{
                          background: `var(--color-${
                            ctemHonesty.showPercent
                              ? (STAGE_STATUS_TONE[s.status] ?? "inconclusive")
                              : "inconclusive"
                          })`
                        }}
                      />
