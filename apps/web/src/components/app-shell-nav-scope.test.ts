...
describe("nav scope (P07-17 / UX-W15)", () => {
  ...
  it("defines OPERATING_DEFAULT_NAV with JOBS-SIMPLE spine + collapsed Setup", () => {
    const body =
      source.match(
        /const OPERATING_DEFAULT_NAV = new Set\(\[([\s\S]*?)\]\);/
      )?.[1] ?? "";
    const hrefs = [...body.matchAll(/"(\/[^"]+)"/g)].map((m) => m[1]!);
    // Operate spine literals; Setup infrastructure via spread.
    // JOBS-SIMPLE: Home · Scope · Validate · Findings · Remediate · Evidence.
    // Connect is not a Scope peer (rail CTA is resolveFirstRunPrimaryAction).
  ...
  it("lists OPERATING_SETUP_HIDDEN junk drawer for mature Setup collapse", () => {
    ...
    // or a persona allow-list re-surfaces them.
  ...
  it("does not pin Shift brief or Connect on Operating default (JOBS-SIMPLE)", () => {
    // ENGINEER_DAILY_NAV spreads OPERATING_DEFAULT_NAV (spine, no /shift).
    const operatingBody =
      source.match(
        /const OPERATING_DEFAULT_NAV = new Set\(\[([\s\S]*?)\]\);/
      )?.[1] ?? "";
    expect(operatingBody).not.toMatch(/"\/shift"/);
    expect(operatingBody).not.toMatch(/"\/integrations"/);
  ...
  it("puts Findings, Remediate, Evidence on NEW_TENANT_NAV (empty New Operate rail)", () => {
    // PERISCAN-538: New maturity uses NEW_TENANT_NAV, not OPERATING_DEFAULT_NAV.
    // Empty tenants still need the JOBS-SIMPLE Operate spine (honest empty states).
    const hrefs = setHrefs("NEW_TENANT_NAV");
    expect(hrefs).toEqual(
  ...
  it("keeps a single rail primary CTA from resolveFirstRunPrimaryAction", () => {
    expect(source).toMatch(/resolveFirstRunPrimaryAction\(activation\.data/);
    expect(source).toMatch(/data-testid="rail-primary-cta"/);
