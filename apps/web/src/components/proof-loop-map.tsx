...
export interface ProofLoopMapNode {
  ...
  /**
   * Operator-visible status. Parent-complete Validate is "Measured";
   * never a 1/1 child-engine meter.
   */
  chip: string;
}
...
export type ProofLoopMapOptions = {
  /**
   * Community first-run: Authorize is node 1. Connect is optional extra
   * signal, not the door — do not paint Connect as Now on an empty tenant.
   */
  community?: boolean;
};
...
export function buildProofLoopMapNodes(
  ...
  options?: ProofLoopMapOptions
): ProofLoopMapNode[] {
  const milestones = activation?.milestones ?? [];
  const sourceConnected = milestones.find(
    (milestone) => milestone.key === "SourceConnected"
  );
  // SourceConnected is optional extra signal (PERISCAN-547). Do not paint
  // Connect IN PROGRESS 1/2 from AccountCreated alone after Community proof.
  const skipOptionalConnect =
    Boolean(options?.community) ||
    (sourceConnected != null && sourceConnected.state !== "Completed");
  const catalogEntries = skipOptionalConnect
    ? PROOF_LOOP_HELP.filter((entry) => entry.label !== "Connect")
    : PROOF_LOOP_HELP;
  let currentStage = activation?.currentStage;
  let nextAction = activation?.nextAction;
  const primary = resolveFirstRunPrimaryAction(activation);
  // Watch = parent mission still in flight; MissionCreated 1/1 is not Validate DONE.
  const watchingCommunityRun =
    primary.label === COMMUNITY_FIRST_RUN_WATCH_LABEL;
  const measuredParent = milestones.some(
    (milestone) =>
      milestone.key === "MeasuredResult" && milestone.state === "Completed"
  );
  ...
  if (options?.community) {
    ...
    if (activation) {
      const scopeVerified = milestones.some(
        (milestone) =>
          milestone.key === "ScopeVerified" && milestone.state === "Completed"
      );
      if (!scopeVerified) {
        currentStage = "Authorize";
      } else if (!measuredParent || watchingCommunityRun) {
        currentStage = "Validate";
      }
    }
  } else if (watchingCommunityRun) {
    currentStage = "Validate";
  }
  ...
  return catalogEntries.map((catalog) => {
    ...
    const watchingValidate =
      watchingCommunityRun && catalog.label === "Validate";
    const completedCount = watchingValidate
      ? 0
      : stageMilestones.filter((m) => m.state === "Completed").length;
    const hasBlocked = stageMilestones.some((m) => m.state === "Blocked");
    const hasCurrent = stageMilestones.some((m) => m.state === "Current");
    const isApiCurrent =
      currentStage === catalog.label &&
      completedCount < Math.max(totalCount, 1);

    let state: ProofLoopMapNodeState = "Upcoming";
    if (hasBlocked) {
      state = "Blocked";
    } else if (watchingValidate) {
      state = "Current";
    ...
    const focusMilestone =
      ...
      [...stageMilestones].reverse().find((m) => m.state === "Completed") ??
      null;
...
/** Visible stage chip: parent-complete Validate is Measured; never Done · 1/1. */
export function proofLoopMapChip(node: {
  stage: ProofLoopMapStage;
  state: ProofLoopMapNodeState;
  completedCount: number;
  totalCount: number;
  measuredParent: boolean;
}): string {
  const status =
    node.stage === "Validate" &&
  ...
  if (node.state !== "Current" && node.totalCount > 1) {
    return `${status} · ${node.completedCount}/${node.totalCount}`;
  }
  return status;
}
...
function nodeClasses(state: ProofLoopMapNodeState, compact: boolean): string {
  ...
  switch (state) {
    ...
    case "Current":
      return cn(base, "border-brand-fill bg-brand-fill text-white");
    case "Partial":
      ...
      {nodes.map((node, index) => {
        ...
        return (
          <Link
            ...
            aria-label={`${node.stage} — ${node.chip}. ${node.milestoneLabel ?? node.detail}`}
          ...
          >
            ...
            <span className="sr-only">{node.chip}</span>
          </Link>
        );
      })}
    </div>
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-base font-semibold text-ink">
            {nodes.map((node) => node.stage).join(" → ")}
          </h2>
        ...
        {nodes.map((node, index) => {
          ...
          return (
            <li key={node.stage} className="flex min-w-0 flex-1 items-center">
              <Link
                href={node.href}
                title={`${node.stage} — ${node.chip}`}
                aria-label={`${node.stage}: ${node.chip}`}
                aria-current={node.state === "Current" ? "step" : undefined}
                className={cn(
...
export function ProofLoopMap({
  ...
  const nodes = useMemo(
    ...
    [activation, loading, community]
  );
  const communityPrimary =
    community && !loading ? resolveFirstRunPrimaryAction(activation) : null;
  const nextAction = loading
    ? null
    : communityPrimary
      ? {
          href: communityPrimary.href,
          label: communityPrimary.label,
          reason: communityPrimary.reason ?? ""
        }
