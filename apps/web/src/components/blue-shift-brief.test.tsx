...
describe("BlueShiftBriefPanel (ICP 5.0 residual)", () => {
  ...
  it("keeps continuous health and exposes one Start triage CTA to Active findings", async () => {
    ...
    const programNote = screen.getByText("Morning validated program health.");
    expect(programNote).toBeInTheDocument();
    // Dark surface: slate-600/500 fail WCAG AA; kit tokens pass.
    expect(programNote).toHaveClass("text-muted");
    expect(programNote.className).not.toMatch(/text-slate-/);
    const generated = screen.getByText(/Generated /);
    expect(generated).toHaveClass("text-subtle");
    expect(generated.className).not.toMatch(/text-slate-/);
