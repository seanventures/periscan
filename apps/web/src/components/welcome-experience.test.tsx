...
describe("WelcomeExperience", () => {
  ...
  it("teaches operators authorize → run, not a 3-step connect-first path", async () => {
    const now = "2026-07-14T20:00:00.000Z";
    vi.stubGlobal(
      "fetch",
      vi.fn(async (input: string) => {
    ...
    const guidance = await screen.findByText(
      /Security leaders continue to Executive/u
    );
    expect(guidance).toHaveTextContent(/authorize → run/u);
    expect(guidance).not.toHaveTextContent(
      /connect\s*→\s*authorize\s*→\s*validate/i
    );
    const copy = document.body.textContent ?? "";
    expect(copy).toMatch(/authorize → run/u);
    expect(copy).not.toMatch(/connect\s*→\s*authorize\s*→\s*validate/i);
    expect(copy).not.toMatch(/three Home setup steps/i);
  });
});
