...
export function ValidationSnapshotFlow() {
  ...
  const [scopeType, setScopeType] =
    useState<CommunityValidateScopeType>("Repository");
  ...
  const pinnedModuleIds = useMemo(
    () =>
      typeof window === "undefined"
        ? []
        : parsePinnedCommunityModuleIds(window.location.search),
    []
  );
  const startNowModuleIds = useMemo(
    () =>
      resolveCommunityStartNowModuleIds({
        pinnedModuleIds,
        startableModuleIds: suite.data?.startableModuleIds ?? []
      }),
    [pinnedModuleIds, suite.data?.startableModuleIds]
  );
  const showFullPackControl =
    Boolean(suite.data) &&
    startNowModuleIds.length > 0 &&
    startNowModuleIds.length < (suite.data?.startableModuleIds.length ?? 0);
  const fullPackStartNowCopy = suite.data
    ? communityStartNowCopy({
        cloudAwsAvailable: suite.data.cloudAwsAvailable,
        deferredModules: suite.data.deferredModules,
        pack: "full",
        pinnedModuleIds: [],
  ...
  const communityStartDoor = useMemo(
    () =>
      resolveCommunityStartPrimaryAction({
        cloudAwsAvailable: suite.data?.cloudAwsAvailable,
        startableModuleIds: suite.data ? startNowModuleIds : null
      }),
    [startNowModuleIds, suite.data]
  );
  ...
  async function addScope() {
    ...
    if (looksLikeHostedGitHubUrl(value)) {
      setScopeError(HOSTED_GITHUB_SCOPE_HINT);
      return;
    }
    setAdding(true);
    setScopeError(null);
  ...
  async function verify(operatorAttestation = false) {
    ...
    if (looksLikeHostedGitHubUrl(selectedScope.value)) {
      setScopeError(HOSTED_GITHUB_SCOPE_HINT);
      return;
    }
    setVerifying(true);
  ...
  async function runCommunitySuite(pack: CommunityStartPack = "first-hour") {
    ...
    setRunError(null);
    try {
      const moduleIds = communityStartRequestModuleIds({
        pack,
        pinnedModuleIds: pack === "full" ? [] : pinnedModuleIds,
        startableModuleIds: suite.data?.startableModuleIds ?? []
      });
      const started = await api.startCommunityValidation({
        ...
        ...(moduleIds && moduleIds.length > 0 ? { moduleIds } : {}),
        policyDecisionId: policy.policyDecisionId,
        scopeId: selectedScope.scopeId
      });
        ...
        {scopes.loading ? (
          ...
          <div className="flex flex-col gap-3 p-4">
              ...
              <div className="flex flex-wrap items-end gap-2">
                ...
                <button
                  ...
                  disabled={
                    adding ||
                    !newScopeValue.trim() ||
                    looksLikeHostedGitHubUrl(newScopeValue)
                  }
            ...
            {selectedScope && !gateOk ? (
              looksLikeHostedGitHubUrl(selectedScope.value) ? (
                <ScopeValueHint value={selectedScope.value} />
              ) : (
        ...
        <Panel>
          ...
          <div className="flex flex-col gap-3 p-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {!showConnectAwsPrimary && !hideDeadRunButton ? (
                <button
                  type="button"
                  onClick={() => void runCommunitySuite()}
                  disabled={
                    !gateOk ||
                    !policyAllowsRun ||
                    running ||
