...
/**
 * Planning ranges for first MeasuredResult. Connect is not a first-run door.
 */
...
/**
 * Hide first-proof percent until the tenant has real activation work.
 * AccountCreated-only is 1/6 = 17% theater on an empty Community tenant.
 */
export function showFirstMeasuredProofPercent(
  activation: ProductActivationState | null | undefined
): boolean {
  if (!activation) {
    return false;
  }
  if (activation.completedMilestones > 1) {
    return true;
  }
  return activation.milestones.some(
    (milestone) =>
      milestone.key === "MeasuredResult" &&
      (milestone.state === "Current" || milestone.state === "Completed")
  );
}
 ...
 * path to first value: authorize a scope, then run Community validation. A
 * vendor source is optional extra signal, not the door. Progress is REAL
 * (derived from scopes and snapshots), so a step checks itself off as the user
...
export function GetStarted({ userName }: { userName?: string }) {
  ...
  const actionableDiagnostics = (activation.data?.diagnostics ?? []).filter(
    (diagnostic) => {
      if (diagnostic.code === "experience_profile_incomplete") {
        return false;
      }
      // Community first-run does not require a vendor source — do not pair
      // source-missing as equal ATTENTION with no-scope.
      if (
        primary.setupIncomplete &&
        (diagnostic.code === "source_missing" ||
          diagnostic.code === "source_not_connected")
      ) {
        return false;
      }
      return true;
  ...
  return (
    <div className="relative mx-auto max-w-6xl px-6 py-10 md:px-10 md:py-14">
        ...
        <div className="max-w-xl">
          ...
          <p className="mt-4 text-[15px] leading-relaxed text-muted">
            {setupComplete
              ...
              : `${userName ? `${userName.split(" ")[0]}, two` : "Two"} steps take you from an empty console to a measured, evidence-backed snapshot of what an attacker can actually reach — and proof of what you've fixed.`}
          </p>
          ...
          >
            <div
              ...
              aria-valuenow={setupComplete ? completedMilestones : completed}
              aria-label={
                setupComplete
      ...
      >
        ...
        <div className="space-y-4 border-t border-line px-4 py-4">
          <FirstMeasuredProofCountdown
            ...
            showPercent={showFirstMeasuredProofPercent(activation.data)}
          />
          <FirstRunTtvStrip
      ...
      {!setupComplete && !sourceConnected ? (
        <p className="mt-4 max-w-2xl text-[13px] leading-relaxed text-subtle">
          A vendor connector is optional extra signal — Community starts on
          verified scope.{" "}
          <Link
            href="/integrations"
            className="font-semibold text-muted underline underline-offset-2 hover:text-ink"
            data-testid="get-started-connect-optional"
          >
            Optional: connect a source
          </Link>
        </p>
      ) : null}
      ...
      {hasValidation ? (
        ...
        >
          ...
          <p className="mt-1 max-w-2xl text-xs leading-5 text-muted">
            After a snapshot correlates paths, open Attack paths and Measure hop
            (safe) on each edge. Allowed policy auto-queues the probe; confirm
            progress only via edge receipts with evidence IDs — never invent
            Measured certainty from launch status.
          </p>
          <div className="mt-3 flex flex-wrap gap-3">
            <Link
              href="/attack-paths"
...
function FirstMeasuredProofCountdown({
  ...
  const percentValue = !loading && showPercent ? progress.percent : 0;
  return (
    <section
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        >
          {loading ? "…" : showPercent ? `${progress.percent}%` : "—"}
        </p>
      </div>
      <div
        ...
        {progress.measured
          ...
          : loading
            ...
            : showPercent
              ...
              : "No measured progress yet. Authorize a scope, then run a real check."}
      </p>
      {sourceConnected ? (
