...
/** Generic motion+class titles (e.g. "EXV SecretExposure") are not operator identity. */
const GENERIC_MOTION_TITLE =
  /^(EXV|BAS|AIApp|Cloud|Secrets|APT|DRV|CSV|SCV|ASV)\s+\S+$/;
...
/**
 * Collapsed-row identity: file path / location + rule/class, not fingerprint.
 * One leak (leaked.js + slack-app-token) must be distinguishable from EXV soup.
 */
function resolveFindingRowIdentity(finding: ValidatedFinding): {
  title: string;
  location: string | null;
  rule: string | null;
} {
  const location = finding.location?.trim() || null;
  const rule = finding.ruleId?.trim() || null;
  const generic = GENERIC_MOTION_TITLE.test(finding.title.trim());
  if (generic && (location || rule)) {
    const file = location ? findingFileName(location) : null;
    return {
      location,
      rule,
      title: [file, rule].filter(Boolean).join(" · ")
    };
  }
...
export function FindingsWorkbench() {
  ...
  const hasOperatorFilters =
    ...
    savedView === "priority-unowned" ||
    savedView === "my-queue" ||
    savedView === "new-untriaged" ||
    savedView === "reopened";
  // Hide views/filters/how-queue on first-run empty; keep them on a filter miss.
  const showFilterChrome = hasFindings || hasOperatorFilters;
  ...
  const filtered = useMemo(() => {
    ...
    return [...all]
      ...
      .filter(
        (f) =>
          !q ||
          `${f.title} ${f.impact} ${f.remediation} ${f.location ?? ""} ${f.ruleId ?? ""}`
            .toLowerCase()
            .includes(q)
      )
  ...
  return (
    ...
    >
          ...
          <ul className="mt-3 list-none divide-y divide-line border-t border-line">
            {governedAcceptances.slice(0, 5).map((finding) => (
              ...
              >
                ...
                >
                  {resolveFindingRowIdentity(finding).title}
                </button>
      ...
      {(() => {
        ...
        ) : all.length === 0 && !hasOperatorFilters ? (
          <div className="p-4" data-testid="findings-empty">
            <EmptyState
              title="No findings yet"
              description="An empty queue is not a clean bill of health — run Community validation to produce evidence-backed findings."
              action={
                <Link
                  href="/missions"
        ...
        if (selectedFinding) {
          const selectedIdentity = resolveFindingRowIdentity(selectedFinding);
          return (
            <div
            ...
            >
              ...
              <Panel
                ...
                role="region"
                aria-label={selectedIdentity.title}
                aria-keyshortcuts="ArrowUp ArrowDown Escape"
...
function isFindingRowControl(target: EventTarget | null): boolean {
  return target instanceof Element
    ? Boolean(target.closest("input, a, select, textarea"))
    : false;
}
...
function FindingRow({
  ...
  return (
    <li
      ...
      tabIndex={0}
      aria-label={`Finding: ${identity.title}`}
      onClick={(event) => {
        if (isFindingRowControl(event.target)) return;
        onToggle();
      }}
      onKeyDown={(event) => {
      ...
      className={cn(
        "last:border-b-0 cursor-pointer rounded-control focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 focus-visible:ring-offset-bg",
    ...
    >
      ...
      >
        ...
        <button
          ...
          aria-label={`Expand finding: ${identity.title}`}
          className={cn(
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <div className="flex min-w-0 flex-wrap items-center gap-2">
              ...
              {identity.location ? (
                <span
                  data-testid={`finding-location-${finding.findingId}`}
                  title={identity.location}
                  className="shrink-0 font-mono text-[11px] tracking-tight text-muted"
                >
                  {identity.location}
                </span>
              ) : null}
              {identity.rule ? (
                <span
                  data-testid={`finding-rule-${finding.findingId}`}
                  title={identity.rule}
                  className="shrink-0 font-mono text-[11px] tracking-tight text-muted"
                >
                  {identity.rule}
    ...
    <div className="flex flex-col gap-4 bg-surface/40 px-4 py-4">
      ...
      {(finding.fingerprint ||
        ...
        <div className="rounded-card border border-line p-3">
          ...
          <div className="mt-2 flex flex-wrap gap-x-6 gap-y-2 text-[12px]">
            {finding.location ? (
              <Meta label="Location" value={finding.location} />
            ) : null}
            {finding.ruleId ? (
              <Meta label="Rule" value={finding.ruleId} />
            ) : null}
            {finding.fingerprint ? (
              <Meta
                label="Fingerprint"
