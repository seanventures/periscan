...
const NEW_TENANT_NAV = new Set([
  ...
  "/runners",
  "/missions",
  // PERISCAN-538: empty New Operate rail still shows the JOBS-SIMPLE spine
  // (honest empty Findings / Remediate / Evidence). Connect stays Setup.
  "/findings",
  "/remediation",
  "/evidence",
  "/trust-safety"
 ...
 * JOBS-SIMPLE: Scope stays on Operate; Connect is Setup (rail CTA is
 * resolveFirstRunPrimaryAction). Schedule lives under Setup and is
 * re-surfaced for SecurityEngineer daily when needed.
 */
const OPERATING_SETUP_NAV = new Set(["/runners", "/engines"]);

/**
 * JOBS-SIMPLE Operate spine (Home · Scope · Validate · Findings · Remediate ·
 * Evidence) plus collapsed Setup infrastructure (Runners · Engines). Must
 * match PRIMARY_NAV Operate. Connect is not an Operate peer of Scope.
 *
 ...
 * portal-only via Show Labs & more (or persona allow-lists).
 */
...
const ENGINEER_DAILY_NAV = new Set([
  ...
  // UX-W15: Schedule if needed; Evidence is already on the Operate spine.
  "/schedules",
  "/evidence",
  "/trust-safety"
]);
...
function RailNav({
   ...
   * - Mature + no persona: Operate rail (JOBS-SIMPLE spine)
