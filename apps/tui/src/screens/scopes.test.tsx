...
describe("ScopesScreen", () => {
  ...
  it("shows the repository verification token and auth filename for a pending repo", async () => {
    delete process.env.PERISCAN_LAB_MODE;
    const pending = scope({
      scopeId: "33333333-3333-4333-8333-333333333333",
      scopeType: "Repository",
      value: "/tmp/tui-first-hour",
      verificationToken: "tok-repo-file"
    });
    const api = fakeApi({
      listScopes: vi.fn().mockResolvedValue([pending])
    });
    ...
    await frameContains(instance, "/tmp/tui-first-hour");
    const frame = instance.lastFrame() ?? "";
  ...
  it("reports create-mode typing so the app can trap digits only while entering a value", async () => {
