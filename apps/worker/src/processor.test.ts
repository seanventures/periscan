...
const HASH_PATH_UUID =
  /^[0-9a-f]{8}-[0-9a-f]{4}-5[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

const GITLEAKS_FIXTURE_ROOT = path.resolve(
  path.dirname(fileURLToPath(import.meta.url)),
  "../../../packages/modules/fixtures"
);

function gitleaksFixtureTarget(
  overrides: Record<string, unknown> = {}
): Record<string, unknown> {
  return {
    fixtureMode: true,
    fixtureReportPath: path.join(
      GITLEAKS_FIXTURE_ROOT,
      "gitleaks/repo-secret-fixture.json"
    ),
...
function pinTenant(
  created: ReturnType<typeof createExecution>,
  tenantId: string
) {
  created.execution.job.tenantId = tenantId;
  created.execution.mission.tenantId = tenantId;
  created.execution.run.tenantId = tenantId;
  created.payload.tenantId = tenantId;
  return created;
}
...
describe("mission execution processor", () => {
  ...
  it("processes a queued job and persists normalized signals", async () => {
    ...
    expect(
      store.persistedSignals.every((signal) => signal.relatedPathIds.length === 0)
    ).toBe(true);
  ...
  it("attaches a stable secret-to-asset relatedPathId for gitleaks.repo_secrets", async () => {
    const tenantId = "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa";
    const first = pinTenant(
      createExecution("gitleaks.repo_secrets", gitleaksFixtureTarget()),
      tenantId
    );
    const store = new InMemoryMissionExecutionStore(first.execution);
    const result = await createMissionExecutionProcessor(store, {
      allowFixtureTargets: true
    }).process(first.payload);

    expect(result.validationState).toBe("Validated");
    expect(result.signals.length).toBeGreaterThan(0);
    const pathIds = result.signals.map((signal) => signal.relatedPathIds);
    expect(pathIds.every((ids) => ids.length === 1)).toBe(true);
    const pathId = pathIds[0]?.[0];
    expect(pathId).toMatch(HASH_PATH_UUID);
    expect(pathIds.every((ids) => ids[0] === pathId)).toBe(true);
    expect(store.persistedSignals.map((signal) => signal.relatedPathIds[0])).toEqual(
      result.signals.map(() => pathId)
    );

    const second = pinTenant(
      createExecution("gitleaks.repo_secrets", gitleaksFixtureTarget()),
      tenantId
    );
    const secondResult = await createMissionExecutionProcessor(
      new InMemoryMissionExecutionStore(second.execution),
      { allowFixtureTargets: true }
    ).process(second.payload);
    expect(secondResult.signals[0]?.relatedPathIds[0]).toBe(pathId);

    const otherRepo = pinTenant(
      createExecution(
        "gitleaks.repo_secrets",
        gitleaksFixtureTarget({
          repositoryName: "periscan-fixtures/other-app"
        })
      ),
      tenantId
    );
    const otherResult = await createMissionExecutionProcessor(
      new InMemoryMissionExecutionStore(otherRepo.execution),
      { allowFixtureTargets: true }
    ).process(otherRepo.payload);
    expect(otherResult.signals[0]?.relatedPathIds[0]).toMatch(HASH_PATH_UUID);
    expect(otherResult.signals[0]?.relatedPathIds[0]).not.toBe(pathId);
  });

  it("uses a hop-bound attackPathId for gitleaks instead of synthesizing a path", async () => {
    const attackPathId = randomUUID();
    const { execution, payload } = createExecution(
      "gitleaks.repo_secrets",
      gitleaksFixtureTarget({ attackPathId, pathEdgeId: randomUUID() })
    );
    const result = await createMissionExecutionProcessor(
      new InMemoryMissionExecutionStore(execution),
      { allowFixtureTargets: true }
    ).process(payload);

    expect(result.signals.length).toBeGreaterThan(0);
    expect(
      result.signals.every((signal) => signal.relatedPathIds[0] === attackPathId)
    ).toBe(true);
  });

  it("does not overwrite gitleaks relatedPathIds already set on the signal", async () => {
    const existingPathId = randomUUID();
    const timestamp = new Date().toISOString();
    const execute = vi.spyOn(modules, "executeModuleById").mockResolvedValue({
      errors: [],
      evidence: [
        {
          ...
          attributes: {
            fingerprint: "periscan-fixture-redacted-secret-fingerprint",
            repositoryName: "periscan-fixtures/demo-app"
          },
      ...
      summary: "Gitleaks found 1 potential secret exposure in fixture.",
    ...
    try {
      ...
      expect(result.signals[0]?.relatedPathIds).toEqual([existingPathId]);
    } finally {
      execute.mockRestore();
    }
  });
