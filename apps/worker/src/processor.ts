...
/**
 * Attach a real hop-bound AttackPath id only. Do not mint a hash UUID —
 * relatedPathIds must name an AttackPath row or claim-safe UI remaps
 * Validated → Discovered without hop receipts (PERISCAN-568).
 */
function attachGitleaksSecretToAssetPathIds(
  ...
  return signals.map((signal) =>
    signal.relatedPathIds.length > 0
      ? signal
      : { ...signal, relatedPathIds: [hopBoundPathId] }
  );
}
