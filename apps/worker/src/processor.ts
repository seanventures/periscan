...
function readRepositoryLabel(target: Record<string, unknown>): string {
  const name = readString(target, "repositoryName");
  if (name) {
    return name;
  }
  const repositoryPath = readString(target, "repositoryPath");
  return repositoryPath ? pathBasename(repositoryPath) : "";
}

function readHopBoundPathId(target: Record<string, unknown>): string | null {
  const candidate =
    readString(target, "attackPathId") ?? readString(target, "pathId");
  return candidate && isUuid(candidate) ? candidate : null;
}

function readSecretFingerprints(result: ModuleOutput): string[] {
  const fingerprints: string[] = [];
  for (const evidence of result.evidence) {
    const fingerprint = readString(asRecord(evidence.attributes), "fingerprint");
    if (fingerprint) {
...
/** Hash-based UUID v5-shaped id for a secret→asset correlation. Not an AttackPath row. */
function computeGitleaksSecretToAssetPathId(input: {
...
function attachGitleaksSecretToAssetPathIds(
  moduleId: string,
  target: Record<string, unknown>,
  tenantId: string,
  result: ModuleOutput,
  ...
  const hopBoundPathId = readHopBoundPathId(target);
  const fingerprints = readSecretFingerprints(result);
  if (!hopBoundPathId && fingerprints.length === 0) {
    return signals;
  }

  const pathId =
    hopBoundPathId ??
    computeGitleaksSecretToAssetPathId({
      repositoryLabel: readRepositoryLabel(target),
