...
export function buildVerificationResult(input: {
   ...
   * `no_secret_exposure_observed` / `Fixed` retest with originating evidence is
   * the verification event. Omitted for path-based verify.
   */
  ...
  if (!input.currentDraft) {
    ...
    // clear negative (Gitleaks `no_secret_exposure_observed`) is that
    // verification event for Community findings that never had a path.
