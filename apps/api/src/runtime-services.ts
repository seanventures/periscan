...
import type {
  ...
  AssemblePassiveMultiAgentPlanInput,
  AssemblePassiveMultiAgentPlanResponse,
  BatchComplianceGovernanceInput,
  BatchComplianceGovernanceResult,
  CommunityMissionRemediationsResult,
  CommunityValidationCompanion,
  CommunityValidationStartResult,
  CommunityValidationSuiteResponse,
  CompileHybridExecutionInput,
  CompileHybridExecutionResponse,
  ComplianceGovernanceMultiFrameworkSummary,
  ConversationalMissionDraft,
  ConvertMissionDraftToHybridCompileInputRequest,
  CreateConversationalMissionDraftInput,
  ExecutionIntegrityHonesty,
  HybridCompileInputFromDraft,
  ModelExtractionHonesty,
  MultiFrameworkComplianceExportInput,
  MultiFrameworkComplianceExportResult,
  PartnerCapabilityHonesty,
  SafetyEquivalentPacksResponse,
  StartCommunityValidationRequest
} from "@periscan/shared";
...
function isTriggerDisabledByParameters(
  triggerId: string,
  ruleParameters: Record<string, unknown> | undefined
): boolean {
  const raw = ruleParameters?.[triggerId];
...
export function buildSignalTriggerEvaluationResponse(
  ...
  const rules = SIGNAL_TRIGGER_RULES.map((rule) => ({
    ...rule,
    enabled:
      rule.enabled &&
      !isTriggerDisabledByParameters(rule.triggerId, state.ruleParameters)
  }));
  const evaluations = rules.map((rule) => {
    if (!rule.enabled) {
      return {
        ...
        matchedSignalIds: [],
        missingPrerequisites: [],
        reason: `${rule.name} is disabled by tenant rule parameters.`,
...
export function getInterventionSigningSecret(
  env: NodeJS.ProcessEnv = process.env
) {
  if (env.PERISCAN_INTERVENTION_SIGNING_SECRET) {
    return env.PERISCAN_INTERVENTION_SIGNING_SECRET;
  }

  if (env.PERISCAN_DEPLOYMENT_ENVIRONMENT === "production") {
    throw new AppServiceError(
      "Set PERISCAN_INTERVENTION_SIGNING_SECRET in production; refusing to sign intervention tokens with the session JWT secret.",
      503,
      "intervention_signing_secret_missing"
    );
  }

  return env.PERISCAN_JWT_SECRET ?? "periscan-dev-intervention-secret";
}

const PRODUCTION_SECRET_PLACEHOLDER = /^(todo|changeme|replace-me|placeholder)$/iu;

export function assertProductionSecretsAtRest(
  env: NodeJS.ProcessEnv = process.env
) {
  if (env.PERISCAN_DEPLOYMENT_ENVIRONMENT !== "production") {
    return;
  ...
  for (const key of [
    ...
    if (!value || PRODUCTION_SECRET_PLACEHOLDER.test(value)) {
      throw new Error(
...
export const AUDIT_ACTION_TO_DB = {
  ...
  "third_party_tool.uninstalled": "third_party_tool_uninstalled",
...
const DEPLOYMENT_CONFIG_DEFINITIONS: DeploymentConfigDefinition[] = [
  ...
  {
    key: "PERISCAN_INTERVENTION_SIGNING_SECRET",
    label: "Model-tool intervention token signing secret",
    category: "Security",
    required: true
  },
  {
...
function parseFindingLocationMeta(signal: SignalEnvelope): {
  location: string | null;
  ruleId: string | null;
} {
  const pointer = signal.rawPayloadPointer ?? "";
  if (!pointer) {
    return { location: null, ruleId: null };
  }
  try {
    ...
    const ruleId =
      ...
      url.searchParams.get("ruleId") ??
      url.searchParams.get("class");
    const line = url.searchParams.get("line");
    const trimmedFile = file?.trim() || null;
    const trimmedRule = ruleId?.trim() || null;
    const location =
      trimmedFile && line && /^\d+$/u.test(line)
        ? `${trimmedFile}:${line}`
        : trimmedFile;
    return { location, ruleId: trimmedRule };
  } catch {
    return { location: null, ruleId: null };
  }
}

function locationFileName(location: string): string {
  const trimmed = location.trim();
  const parts = trimmed.split(/[\\/]/u);
  return parts[parts.length - 1] || trimmed;
}

function distinguishSignalFindingTitle(input: {
  location: string | null;
  ruleId: string | null;
  sourceMotion: string;
  subcategory: string;
}): string {
  const file = input.location ? locationFileName(input.location) : null;
  if (file && input.ruleId) {
    return `${file} · ${input.ruleId}`;
  }
  if (file) {
    return file;
  }
  ...
  return `${input.sourceMotion} ${input.subcategory}`;
}
...
export function filterValidatedFindings(
  ...
  return findings.filter((finding) => {
    ...
    if (
      filters.search &&
      !`${finding.title} ${finding.impact} ${finding.remediation} ${finding.location ?? ""} ${finding.ruleId ?? ""}`
        .toLowerCase()
...
export function buildValidatedFindings(input: {
  ...
  for (const signal of input.signals) {
    ...
    const locationMeta = parseFindingLocationMeta(signal);
    const signalFingerprint = computeSignalFindingMaterial({
      correlationKeys: [
        ...extractSignalCorrelationKeys(signal),
        ...(locationMeta.location ? [`file:${locationMeta.location}`] : []),
        ...(locationMeta.ruleId ? [`rule:${locationMeta.ruleId}`] : [])
      ],
    ...
    findings.push({
      ...
      title: imported
        ...
        : distinguishSignalFindingTitle({
            location: locationMeta.location,
            ruleId: locationMeta.ruleId,
            sourceMotion,
            subcategory: signal.signalSubcategory ?? signal.signalCategory
          }),
      ...(locationMeta.location ? { location: locationMeta.location } : {}),
      ...(locationMeta.ruleId ? { ruleId: locationMeta.ruleId } : {}),
...
/**
 * Originating-module measured retest (Community Gitleaks and other
 * fingerprint-only findings). A clear negative with originating evidence is a
 * verification event even when `relatedPathId` is null.
 */
export type OriginatingModuleRetest = {
  evidenceIds: readonly string[];
  outcome: string | null;
  validationState: string | null;
};

const CLEAR_NEGATIVE_ORIGINATING_RETEST_OUTCOME = "no_secret_exposure_observed";
const ORIGINATING_EXPOSURE_STILL_OBSERVED_OUTCOME = "secret_exposure_observed";

export function selectOriginatingModuleRetest(input: {
  evidenceIds: readonly string[];
  retestRuns: ReadonlyArray<{
    outcome: string | null;
    status: string;
    validationState: string | null;
  }>;
}): OriginatingModuleRetest | null {
  if (input.evidenceIds.length === 0) {
    return null;
  }

  const completed = input.retestRuns.filter((run) => run.status === "Completed");
  if (completed.length === 0) {
    return null;
  }

  const stillObserved = completed.find(
    (run) => run.outcome === ORIGINATING_EXPOSURE_STILL_OBSERVED_OUTCOME
  );
  if (stillObserved) {
    return {
      evidenceIds: input.evidenceIds,
      outcome: stillObserved.outcome,
      validationState: stillObserved.validationState
    };
  }

  const clearNegative = completed.find(
    (run) =>
      run.outcome === CLEAR_NEGATIVE_ORIGINATING_RETEST_OUTCOME &&
      isClearNegativeOriginatingRetestState(run.validationState)
  );
  const chosen = clearNegative ?? completed[0]!;
  return {
    evidenceIds: input.evidenceIds,
    outcome: chosen.outcome,
    validationState: chosen.validationState
  };
}

function hasOriginatingRetestEvidence(
  retest: OriginatingModuleRetest | null | undefined
): retest is OriginatingModuleRetest {
  return Boolean(retest && retest.evidenceIds.length > 0);
}

function isClearNegativeOriginatingRetestState(
  validationState: string | null | undefined
): boolean {
  // Module first-clean is Validated (PERISCAN-571). Historic retests may still
  // persist Fixed. Either is a measured clear negative; Inconclusive is not.
  return validationState === "Fixed" || validationState === "Validated";
}

function isClearNegativeOriginatingRetest(
  retest: OriginatingModuleRetest | null | undefined
): boolean {
  return (
    hasOriginatingRetestEvidence(retest) &&
    retest.outcome === CLEAR_NEGATIVE_ORIGINATING_RETEST_OUTCOME &&
    isClearNegativeOriginatingRetestState(retest.validationState)
  );
}

function isOriginatingExposureStillObserved(
  retest: OriginatingModuleRetest | null | undefined
): boolean {
  return (
    hasOriginatingRetestEvidence(retest) &&
    retest.outcome === ORIGINATING_EXPOSURE_STILL_OBSERVED_OUTCOME
  );
}
...
export function buildVerificationResult(input: {
  ...
  /**
   * Measured originating-module retest for findings with no attack path.
   * Community Gitleaks findings often have `relatedPathIds: []`; a completed
   * `no_secret_exposure_observed` retest (Validated or Fixed) with originating
   * evidence is the verification event. Omitted for path-based verify.
   */
  originatingModuleRetest?: OriginatingModuleRetest | null;
}) {
  ...
  if (!input.currentDraft) {
    // Path disappearance cannot prove a fix without a prior Measured path.
    // A measured originating-module retest with originating evidence and a
    // clear negative (Gitleaks `no_secret_exposure_observed`, Validated or
    // Fixed) is that verification event for Community findings that never had
    // a path.
    if (!input.previousPath) {
      if (isClearNegativeOriginatingRetest(input.originatingModuleRetest)) {
        return {
          newState: "Fixed" as const,
          outcome: "Fixed" as const
        };
      }
      if (isOriginatingExposureStillObserved(input.originatingModuleRetest)) {
        return {
          newState: "StillExposed" as const,
...
export function createRuntimeServices(
  ...
  return services as unknown as AppServices;
}
