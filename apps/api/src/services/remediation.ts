...
function buildFixVerificationRunTarget(input: {
  ...
  if (input.module.manifest.requiredInputs.includes("repositoryPath")) {
    // Fix verification must retest the authorized repository. Lab/devMode
    // fixture substitution always reports the canned leak, so a cleaned
    // customer repo can never reach RemediationTask Fixed (PERISCAN-573).
    target.repositoryPath = input.scope.value;
  }

  if (input.module.manifest.moduleId === "gitleaks.repo_secrets") {
    target.repositoryName =
      input.previousPath?.pathNodes.find((node) =>
        node.label.toLowerCase().includes("repo")
      )?.label ??
      input.scope.value.split("/").filter(Boolean).at(-1) ??
      "repository";
  }
...
export function createRemediationServices(
  ...
  return {
    ...
    async autoRevalidate(this: AppServices, context, remediationId) {
    ...
    async autoMitigate(this: AppServices, context, remediationId) {
    ...
    async verifyRemediation(context, remediationId, requestId) {
      ...
      const originatingModuleRetest = selectOriginatingModuleRetest({
        evidenceIds: remediation.evidenceIds,
        retestRuns: retestRunRecords
      });
    ...
    async runDueReverifications(this: AppServices, context) {
      requireRole(
        context.membership.role,
        SCOPE_EDITOR_ROLES,
