...
export async function tryAutoApplyPathEdgeReceiptFromCompletedRun(
  ...
  const parsedValidationState = ValidationStateSchema.safeParse(
    input.validationState
  );
  const validationState: ValidationState = isIdentityImport
    ? "Inconclusive"
    : parsedValidationState.success
      ? parsedValidationState.data
      : "Inconclusive";
  const outcome =
    input.outcome ??
...
export function fingerprintDispositionKey(fingerprint: string): string {
  return findingDispositionFingerprintKey(fingerprint);
}

export function resolveFindingDispositionOverride(
  finding: {
    findingId: string;
    fingerprint?: string | null;
    memberFindingIds?: string[];
  },
  dispositions: Map<string, FindingDispositionOverride>
): (FindingDispositionOverride & { inheritedFromFindingId?: string }) | null {
  const keys: string[] = [];
  ...
  push(finding.findingId);
  for (const memberId of finding.memberFindingIds ?? []) {
    push(memberId);
  }
  if (finding.fingerprint) {
    push(fingerprintDispositionKey(finding.fingerprint));
  }

  let best: { key: string; override: FindingDispositionOverride } | null = null;
  for (const key of keys) {
    const override = dispositions.get(key);
    if (!override) {
      continue;
    }
    if (!best || override.updatedAt > best.override.updatedAt) {
      best = { key, override };
    }
  }
  ...
  return {
    ...best.override,
    inheritedFromFindingId: best.key
  };
}
...
> {
  ...
  return {
    ...
    async listDispositionFeedback(
      ...
      const byReasonMap = new Map<FindingDispositionReasonCode | null, number>();
    ...
    async applyPathEdgeReceipt(
      ...
      let runEvidenceIds: string[] | undefined;
