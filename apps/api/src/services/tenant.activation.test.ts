...
describe("product activation Community nextAction", () => {
  ...
  function buildServices(input: {
    ...
    scope?: typeof verifiedScope & { scopeType?: string };
    ...
    const prisma = {
      ...
      integration: {
        ...
        findMany: vi.fn(async () => input.awsIntegrations ?? [])
      },
      scope: {
        findFirst: vi.fn(async () => input.scope ?? verifiedScope)
      },
      policyDecision: {
  ...
  it("does not advertise Run after a verified CloudAccount with no AWS connector", async () => {
    const services = buildServices({
      communityRun: null,
      mission,
      scope: {
    ...
    expect(activation.nextAction.reason).toMatch(
      /no Community engines start without AWS/i
    );
  });

  it("still advertises Run after a verified CloudAccount when AWS is connected", async () => {
    const services = buildServices({
      awsIntegrations: [
        {
  ...
  it("keeps proof-loop nextAction after a measured Community result when SourceConnected is still empty", async () => {
    const runId = randomUUID();
    const services = buildServices({
      communityRun: {
        ...communityRun({ status: "Completed" }),
    ...
    const remediation = activation.milestones.find(
      (milestone) => milestone.key === "RemediationCreated"
    );

    expect(measured?.state).toBe("Completed");
    expect(sourceConnected?.state).not.toBe("Completed");
    expect(sourceConnected?.state).not.toBe("Current");
    expect(remediation?.state).toBe("Current");
    expect(activation.nextAction.href).not.toBe("/integrations");
    expect(activation.nextAction.label).not.toBe("Connect a source");
    expect(activation.nextAction).toMatchObject({
      href: "/remediation",
