...
import {
  ...
  HOSTED_GITHUB_REPOSITORY_REFUSAL,
...
async function listLicensedCopyleftToolIds(
  prisma: {
    toolLicenseAcceptance: {
      findMany: (args: {
        select: { toolId: true };
        where: { tenantId: string; toolId: { in: string[] } };
      }) => Promise<Array<{ toolId: string }>>;
    };
  },
  tenantId: string
): Promise<string[]> {
  ...
  return {
    ...
    async startMission(context, missionId, input, requestId) {
      ...
      const enrolledRunnerWhere = {
        ...
        status: {
          notIn: ["Revoked", "KillSwitchActive"] as Array<
            "Revoked" | "KillSwitchActive"
          >
        },
        tenantId: context.tenant.tenantId
      };
      ...
      const {
        ...
        const claimed = await tx.validationMission.updateMany({
          where: {
            missionId: mission.missionId,
            status: {
              in: [...STARTABLE_MISSION_STATUSES] as MissionStatus[]
            },
            tenantId: context.tenant.tenantId
          },
          data: {
    ...
    async verifyScope(context, scopeId, input) {
      ...
      if (looksLikeHostedGitHubUrl(scope.value)) {
        throw new AppServiceError(
          HOSTED_GITHUB_REPOSITORY_REFUSAL,
          400,
          "hosted_github_url_not_verifiable"
        );
      }
    ...
    async createCommunityMissionRemediations(
      ...
      const candidatePathIds = [
        ...new Set(
          findings.flatMap((finding) => {
            const relatedPathId = finding.relatedPathIds?.[0];
            return relatedPathId ? [relatedPathId] : [];
          })
        )
      ];
      const existingPathIds = new Set(
        candidatePathIds.length === 0
          ? []
          : (
              await prisma.attackPath.findMany({
                select: { pathId: true },
                where: {
                  pathId: { in: candidatePathIds },
                  tenantId: context.tenant.tenantId
                }
              })
            ).map((row) => row.pathId)
      ...
      for (const finding of findings) {
        ...
        // Only FK pathId when AttackPath exists; Gitleaks relatedPathIds can be synthetic.
        const relatedPathId = finding.relatedPathIds?.[0];
        const pathId =
          relatedPathId && existingPathIds.has(relatedPathId)
            ? relatedPathId
            : undefined;
    ...
    async startCommunityValidation(
      ...
      const firstHourIds = communityFirstHourStartModuleIds(
        available.map((entry) => entry.moduleId)
      );
      let selected =
        !input.moduleIds?.length && firstHourIds.length > 0
          ? available.filter((entry) => firstHourIds.includes(entry.moduleId))
          : available;
      ...
      const copyleftForScope = listCopyleftOptInSuiteForScopeType(
        ...
        (entry) =>
          entry.toolId !== null && licensedCopyleftTools.includes(entry.toolId)
      );
