  ...
  return {
    ...
    async getProductActivationState(context) {
      ...
      const currentIndex = records.findIndex((record) => {
        ...
        // Vendor source is optional on the Community first-run. An in-flight
        // pack must keep Watch on MeasuredResult, not bounce Home to Connect.
        if (record.key === "SourceConnected" && inFlightCommunityRun) {
          return false;
        }
        return true;
      });
