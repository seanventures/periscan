    ...
    async getProductActivationState(context) {
      ...
      const latestCommunityRun =
        ...
        isCommunityValidationModuleId(latestCommunityCandidate.moduleId)
          ? latestCommunityCandidate
          : null;
      const inFlightCommunityRun = latestCommunityRun
        ? await prisma.validationRun.findFirst({
            orderBy: { updatedAt: "desc" },
            where: {
              missionId: latestCommunityRun.missionId,
              status: { in: ["Queued", "Running"] },
              tenantId
            }
          })
        : null;
      const communityNextAction = resolveCommunityActivationNextAction(
        inFlightCommunityRun ?? latestCommunityRun
      );
      // Sibling evidence must not mark MeasuredResult complete while the
      // parent Community mission still has a Queued or Running run.
      const measuredResult = inFlightCommunityRun ? null : measuredRun;
      ...
      const records = [
        ...
        {
          completedAt: measuredResult?.completedAt ?? null,
          evidenceBasis: measuredResult
            ? `Completed run ${measuredResult.runId} has ${measuredResult.evidenceIds.length} evidence item${measuredResult.evidenceIds.length === 1 ? "" : "s"}.`
            : inFlightCommunityRun
      ...
      const currentIndex = records.findIndex((record) => {
        ...
        // Vendor source is optional extra signal, not the Community door.
        // Never make SourceConnected Current: first-hour Authorize/Validate,
        // in-flight Watch, and post-measure Act/Verify/Prove stay primary.
        if (record.key === "SourceConnected") {
          return false;
        }
        return true;
      });
      ...
      if (!connectedSource) {
        ...
        diagnostics.push({
          ...
          href: "/integrations",
          severity: sourceMissingIsOptional ? "Info" : "Attention",
          title: latestSource
      ...
      let nextAction =
        inFlightCommunityRun ||
        (current.key === "MeasuredResult" && !measuredResult)
          ? communityNextAction
          : {
              href: current.href,
      ...
      // PERISCAN-556: verified CloudAccount without AWS cannot queue
      // Community engines. Do not advertise Run as if it will start.
      if (
        nextAction.label === COMMUNITY_FIRST_RUN_START_LABEL &&
        verifiedScope?.scopeType === "CloudAccount"
      ) {
        ...
        if (!selectConnectedAwsIntegrationForProwler(connected)) {
          nextAction = {
            href: "/integrations",
            label: COMMUNITY_FIRST_RUN_CONNECT_AWS_LABEL,
            reason: COMMUNITY_FIRST_RUN_CONNECT_AWS_REASON
          };
        }
      }
    ...
    async getProductWorkQueue(this: AppServices, context) {
      ...
      const [
        ...
        prisma.remediationTask.findMany({
          orderBy: { dueAt: "asc" },
          select: { dueAt: true, recommendedAction: true, remediationId: true },
          where: {
            dueAt: { lt: new Date() },
        ...
        prisma.remediationTask.findMany({
          orderBy: { updatedAt: "asc" },
          select: { recommendedAction: true, remediationId: true, updatedAt: true },
      ...
      for (const rem of readyForRetest.slice(0, 4)) {
        pushFeed({
          ...
          severity: null,
          stage: "Verify",
          title: rem.recommendedAction || "Fix ready for re-test",
          urgency: "Soon"
        });
      }
      ...
      for (const rem of overdue.slice(0, 4)) {
        pushFeed({
          ...
          severity: null,
          stage: "Act",
          title: rem.recommendedAction || "Remediation overdue",
          urgency: "Now"
        });
      }
