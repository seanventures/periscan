...
describe("BILLING_PACKAGE_CATALOG Enterprise coverage", () => {
  ...
  it("defines Enterprise as governance posture over Core/Control foundation (not full-catalog superset)", () => {
    ...
    // Entitlement evaluation is a technical superset; GTM copy stays
    // governance/deployment posture rather than a public full-catalog dump.
    for (const capability of [
    ...
    expect(enterprisePackage?.includedMeterNames).toEqual(
    ...
    expect(enterprisePackage?.publicPricingLanguage.toLowerCase()).toMatch(
      /governance|contact sales|not a public full-catalog/
    );
  });
...
describe("buildValidatedFindings fingerprint grouping (Slice 4 C–D)", () => {
  ...
  it("titles one Gitleaks leak from file path and rule, not EXV SecretExposure", () => {
    const tenantId = randomUUID();
    ...
    const signal = {
      ...
      rawPayloadPointer:
        "gitleaks://lab-repo?file=leaked.js&rule=slack-app-token",
      relatedAssetIds: [],
      signalCategory: "Exposure" as const,
    ...
    expect(findings).toHaveLength(1);
    expect(findings[0]!.title).toBe("leaked.js · slack-app-token");
    expect(findings[0]!.location).toBe("leaked.js");
    expect(findings[0]!.ruleId).toBe("slack-app-token");
    expect(findings[0]!.status).not.toBe("Fixed");
    expect(findings[0]!.validationState).not.toBe("Fixed");
    expect(JSON.stringify(findings[0])).not.toMatch(/xoxb-/);
  });
});
...
describe("PERISCAN-7 one remediation per grouped cause + owner/SLA", () => {
  ...
  it("treats unowned via ownerId/ownerDisplay; Escalated disposition owner is overlay-only; AcceptedRisk acceptor does not count", () => {
    ...
    // Isolated helper uses ownerId/ownerDisplay only. Overlay copies Escalated
    // disposition.ownerId onto finding.ownerId before list/filter (P06-3).
    expect(
      isUnownedValidatedFinding({
        ...base,
        disposition: {
