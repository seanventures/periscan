...
describe("Community CloudAccount suite and start services", () => {
  ...
  it("unpinned Repository start (omit moduleIds) queues Gitleaks first-hour, not the full pack", async () => {
    const { services } = createCommunityServices({
      scope: repositoryScope()
    });
    vi.spyOn(services, "createMission").mockResolvedValue({
    ...
    const startMission = vi.spyOn(services, "startMission").mockImplementation(
      async (_context, _missionId, input) =>
        ({
          jobsQueued: (input as { moduleIds?: string[] }).moduleIds?.length ?? 0,
          mission: { missionId: MISSION_ID, status: "Queued" },
          runs: []
        }) as never
    );
    ...
    expect(started.moduleIds).toEqual(["gitleaks.repo_secrets"]);
    expect(started.jobsQueued).toBe(1);
    expect(startMission.mock.calls[0]?.[2]?.moduleIds).toEqual([
      "gitleaks.repo_secrets"
    ]);
    expect(started.moduleIds).not.toContain("trivy.repo_dependency_scan");
    expect(started.moduleIds).not.toContain("osv.repo_dependency_scan");
    expect(started.moduleIds).not.toContain("detect_secrets.repo_secrets");
  });

  it("empty moduleIds on Repository is the same Gitleaks first-hour set", async () => {
    const { services } = createCommunityServices({
      scope: repositoryScope()
    });
    vi.spyOn(services, "createMission").mockResolvedValue({
  ...
  it("explicit full startable moduleIds still queues the full Community pack", async () => {
    const { services } = createCommunityServices({
      scope: repositoryScope()
    });
    vi.spyOn(services, "createMission").mockResolvedValue({
    ...
    const fullPack = listCommunityValidationStartModules({
      scopeType: "Repository"
    }).map((entry) => entry.moduleId);

    expect(fullPack.length).toBeGreaterThan(1);
    expect(fullPack).toContain("gitleaks.repo_secrets");
    expect(fullPack).toContain("trivy.repo_dependency_scan");
    ...
    expect(started.moduleIds).toEqual(fullPack);
    expect(started.moduleIds).toContain("trivy.repo_dependency_scan");
    expect(started.jobsQueued).toBe(fullPack.length);
    expect(startMission.mock.calls[0]?.[2]?.moduleIds).toEqual(fullPack);
  });

  it("unpinned Domain start still queues the Domain startable set, not Gitleaks", async () => {
    const { services } = createCommunityServices({
      executionEnvironment: "InternalRunner",
      runner: { runnerId: RUNNER_ID },
      safetyLevel: "ActiveNonInvasive",
    ...
    vi.spyOn(services, "startMission").mockImplementation(
      async (_context, missionId, input) =>
        ({
          jobsQueued: (input as { moduleIds?: string[] }).moduleIds?.length ?? 0,
          mission: { missionId, status: "Queued" },
          runs: []
    ...
    expect(started.moduleIds).not.toContain("gitleaks.repo_secrets");
    expect(started.moduleIds.length).toBeGreaterThan(1);
  });
});
