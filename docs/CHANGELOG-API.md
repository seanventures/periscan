...
| Version | Date | Notes |
|---------|------|--------|
| **0.4.1** | 2026-09-18 | High-danger operator gate `GET /bas/danger-catalog` (`getBasDangerOperatorGate`): catalog items plus stored qualification/authorization, never queues. `POST /tenants/current/scim-tokens` (`issueTenantScimToken`) is now in OpenAPI (was `schema.hide`). Bearer `psk_` remains the automation auth path. |
