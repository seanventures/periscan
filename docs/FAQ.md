...
The control plane verifies a **local clone path** it (or an enrolled runner)
can read, via `.periscan-authorization`. It does not OAuth to GitHub and does
not clone `github.com/org/repo` for you.

Hosted GitHub URLs infer Type Repository as a hint, then **refuse** Add /
Verify / Attest (`400` `hosted_github_url_not_verifiable`). Paste the absolute
clone path. Attest is runner-only for a path the plane cannot see — not a skip
for github.com.

### How do I prove a repository?

1. Paste an absolute path (`/opt/customer/repo`).
