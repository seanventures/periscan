...
- **Inbound SCIM 2.0 provisioning of Periscan users and groups ships as a tenant-token SCIM server** under `/api/v1/scim/v2/*` (hashed bearer token, 401 without it). Trust & Safety reports `identityProvisioning.scimInbound.status: Ready` after a token is issued, else `NotConfigured`. Deprovision sets membership Inactive and retains evidence. Login remains SSO or password. Do not claim Okta/Azure SCIM certification or JIT. CyberArk Identity SCIM is a separate **read-only external identity inventory** connector. Advanced custom-role RBAC is not shipped (`BaselineRolesOnly`).
